...
from pyqtgraph import AxisItem, PlotDataItem, PlotItem, ViewBox, mkBrush
...
class MultiAxisPlot(PlotItem):
    """
    MultiAxisPlot is a PyQtGraph PlotItem subclass that has support for adding multiple y axes.
    Currently PyQtGraph does not support this functionality natively, so we do it here by
    ...
    **kargs: optional
    ...
    def __init__(self, parent=None, axisItems=None, **kargs):
        ...
        self.addLegend(offset=(10, 10), labelTextSize='10pt', labelTextColor='black')
    ...
    def addAxis(self, axis, name, plotDataItem=None, setXLink=False, enableAutoRangeX=True, enableAutoRangeY=True,
        ...
        self.axes[str(name)] = {'item': axis,
                                'pos': None}  # The None will become an actual position in rebuildLayout() below
    ...
    def linkDataToAxis(self, plotDataItem: PlotDataItem, axisName: str) -> None:
        ...
        if self.legend is not None:
            self.legend.addItem(plotDataItem, name=plotDataItem._pv_name)
            '''
    ...
    def restoreAxisRanges(self):
        ...
        for axisName, axisValue in self.axes.items():
            ...
            if linkedView is None or axisItem.orientation not in (
            'left', 'right') or axisName not in self.axesOriginalRanges:
                continue
